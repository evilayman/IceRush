...
public class MovementBlast : MonoBehaviour
{
    ...
    private bool gameStarted, leftPressState, rightPressState;
    ...
    void HandRate(bool triggerPressed, ref bool pressState, ref Vector3 handDirection, Vector3 handTransform, ParticleSystem emission, CooldownTimer canAccHand, CooldownTimer canDecHand, ref float currentHandSpeed)
    {

        if (triggerPressed && !pressState)
        {
            pressState = true;
            emission.Play();
        ...
        else if (triggerPressed && pressState)
        {
            currentHandSpeed = 0;
